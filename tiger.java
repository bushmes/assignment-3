...
public class tiger extends Animal
{
    ...
    public void act(Field currentField, Field nextFieldState, int steps)
    {
        ...
        // tiger only act during the day, because they are nocturnal animals
        if(!nightTime(steps)) { 
            if(isAlive()) {
            ...
            // During the night, the tiger does not move or breed, but it can still get infected and spread the disease. We need to update the nextFieldState to reflect any changes in infection status.
            if (isAlive()) {
    ...
    private boolean findMate(Field field)
    {
        ...
        while(it.hasNext()) {
            Location loc = it.next();
            Animal animal = field.getAnimalAt(loc);
            if(animal instanceof tiger Tiger && Tiger.getGender() == 1) {
                if(Tiger.isAlive() && Tiger.getAge() >= BREEDING_AGE) {
                    return true;
                }
            }
