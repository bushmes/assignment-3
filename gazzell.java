...
public class gazzell extends prey
{
            ...
            if(isAlive()) {
                ...
                if(!freeLocations.isEmpty()) {
                    ...
                    for (Animal animal : animals) {
                        if (animal instanceof gazzell Gazzell) {
                            gaz++;
                        }
                    }
                    if (gaz > 10000) { // If there are more than 10000 gazelles in the field, decrease the breeding probability to prevent overpopulation
                        BREEDING_PROBABILITY = 0.1;
                    }else if(gaz <100){
                        BREEDING_PROBABILITY = 0.65; // increase the breeding probability to aviod extinction
                    }else if(gaz <1000){
                        BREEDING_PROBABILITY = 0.28; // increase the breeding probability to encourage population growth when the population is under control
                    } else {
    ...
    public int getEaten() {
        setDead();
        return FOOD_VALUE; // Return a the food value of the eaten gazzell
    }
